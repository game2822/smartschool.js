...
import { GetOIDCAccessTokens, GetOIDCJWKS, GetOIDCWellKnown } from "../routes/OIDC";
...
export class AuthFlow {
    ...
    static async create(school: School, challengeMethod = ChallengeMethod.S256): Promise<AuthFlow> {
        const verifier = generateRandomCode();
        const state = generateRandomCode();
        const metadata = await GetOIDCWellKnown(school.OIDCWellKnown);
        const jwks = await GetOIDCJWKS(metadata.jwks_uri);

        return new AuthFlow(
            challengeMethod,
            {
                wellKnown:             school.OIDCWellKnown,
    ...
    get loginURL(): string {
        const challenge = this.challengeMethod === ChallengeMethod.S256
